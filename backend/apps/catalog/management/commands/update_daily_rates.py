...
class Command(BaseCommand):
    help = 'Update products with daily rates'

    def handle(self, *args, **options):
        self.stdout.write("🔧 Updating product daily rates...")
        ...
        for product_name, rate in product_rates.items():
            try:
                ...
                product.save()
                self.stdout.write(
                    self.style.SUCCESS(f"✅ Updated {product_name}: ${rate}/day")
                )
                updated_count += 1
            except Product.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(f"⚠️  Product not found: {product_name}")
                )
        ...
        for product in products_without_rate:
            ...
            product.save()
            self.stdout.write(
                self.style.SUCCESS(f"✅ Set default rate for {product.name}: ${default_rate}/day")
            )
            updated_count += 1
        
        self.stdout.write(
            self.style.SUCCESS(f"🎯 Updated {updated_count} products with daily rates")
        )
        self.stdout.write(self.style.SUCCESS("✅ All products now have daily rates!"))
